const extractBPValues = (vs) => {
    if (vs.component.length !== 2) {
        throw new Error("Expected exactly two components for systolic and diastolic blood pressures.");
    }

    const systolic = vs.component.find(comp => comp.code.coding.some(c => c.code === '8480-6')).valueQuantity.value;
    const diastolic = vs.component.find(comp => comp.code.coding.some(c => c.code === '8462-4')).valueQuantity.value;
...
const areVitalSignsFromSameDayWithSameValues = (vs1, vs2) => {
    ...
    return sameDay && mmHgClose(bp1.systolic,bp2.systolic) && mmHgClose(bp1.diastolic, bp2.diastolic);
};

const filterRedundantRecords = (newVitalSigns, existingVitalSigns) => {
    return newVitalSigns.filter(nvs => !existingVitalSigns.some(evs => areVitalSignsFromSameDayWithSameValues(nvs, evs)));
};

const calculateDateRange = (vitalSigns) => {
    const dates = vitalSigns.map(vs => new Date(vs.effectiveDateTime || vs.effectivePeriod.start));
